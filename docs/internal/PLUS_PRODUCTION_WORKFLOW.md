...
### 16.11 Real-data engineering tests and NLCD acquisition retry (2026-09-16)

**Evidence distinction.** These are two questions introduced by the research architecture, not two hypotheses claimed by the PLUS authors: (1) does our independent implementation reproduce the intended algorithms/native software behavior, and (2) does the model predict held-out San Antonio land-cover change usefully? The first is implementation verification/fidelity; the second is local predictive validation. Agreement with native PLUS is not evidence that either engine forecasts San Antonio accurately. Because this engine deliberately differs from CARS, comparison should document differences rather than require pixel identity. CARS means **CA based on multi-type random patch seeds**, the PLUS spatial allocation component; LEAS learns expansion suitability, whereas CARS combines suitability with neighborhood/patch dynamics, competition, constraints and demand feedback. Our quota allocator remains PLUS-inspired, not CARS-equivalent. Source: [authors' paper](https://arxiv.org/abs/2010.11541).

**Wuhan test.** Downloaded [official tutorial archive](https://github.com/HPSCIL/Patch-generating_Land_Use_Simulation_Model/blob/master/PLUS_test_data.rar), 50,619,798 bytes, SHA256 `22b2751d235cac72891576a3cc0e5cb2066c7eda5fb60d1d85b5cc6acb82c008`. `7zz` could list but not decompress its RAR method; macOS `bsdtar` extracted successfully into a separate directory, with the partial earlier extraction preserved. Source LULCs and terrain rasters share a 2931×2931, 30 m EPSG:32649 grid, but validity masks differ. No source file was modified.

For the bounded first real-data test, `scripts/run_plus_tutorial_test.py` samples rows/columns 0,4,8,… (120 m, 733×733) and explicitly intersects the 2005/2010/2013/DEM/slope masks, retaining 332,810 cells. It fits 2005→2010 expansion with DEM/slope, 16 trees and seed 42, predicts suitability at 2010, then allocates **observed 2013 class quantities**. Static terrain vintage is unknown; its 2005 config date is an assumption, not verified metadata. Numeric classes 1–7 are retained without a claimed semantic crosswalk; all transitions are permitted for this engineering probe. No real planning restrictions or native tutorial parameter equivalence is implied. Future-reference support is used to define the common evaluation mask, so even the mask is not fully blind. Five-year training and three-year allocation periods differ; no temporal extrapolation skill is tested.

All six stages (fit, predict, allocate, validate, quantity-matched random allocation, random validation) completed experimentally; input and output hashes verified. Terrain-inspired allocation: overall accuracy 77.74%, change Figure of Merit 6.78%; persistence: 80.28%, 0%; quota-matched randomized allocation: 76.80%, 4.26%. Zero quantity disagreement was imposed, not predicted. Only one seed was tested. The random allocator has uniform suitability/no neighborhood preference but retains the same net-change/quota restrictions; it is not a uniform draw from every feasible map. The 26,772 predicted changes versus 65,631 observed changes expose the minimal-net-turnover assumption: balanced losses/gains within classes are not modeled. **Decision: this confirms real-data execution, not forecast readiness.** Gross-turnover support, suitable historical drivers, full-resolution tests, native comparison and held-out evaluation remain unresolved. No tuning against these reported results was performed.

Reproduce after separately downloading/extracting the archive:

```bash
python scripts/run_plus_tutorial_test.py --source PATH/PLUS_test_data --output outputs/wuhan_new_run
```

Output includes one `REPORT.md` per stage, an overview report, configs, source provenance, receipts and validation JSON. The actual final local run is `outputs/plus_tutorial_test_v1/run_120m_v3/`; repeated v2/v3 runs produced the same reported metrics. Outputs/raw files are ignored and not redistributed in Git.

**Annual NLCD retry.** The [official MRLC services page](https://www.mrlc.gov/data-services-page) exposed an Annual NLCD service separate from the legacy MRLC WCS. Its WCS 2.0 GetCoverage request returned XML (`startTime` null) despite HTTP 200; this response is not raster data. WCS **1.0.0** GetCoverage with explicit `time` succeeded for requested 2005, 2015 and 2025, from `https://dmsdata.cr.usgs.gov/geoserver/mrlc_Land-Cover-Native_conus_year_data/wcs`. No requester-pays credentials or charges were used. Coverage name: `mrlc_Land-Cover-Native_conus_year_data:Land-Cover-Native_conus_year_data`; `bbox=-273195,679455,-213075,746835`, `crs=EPSG:5070`, `resx=resy=30`, `format=GeoTIFF`, `time=YEAR-01-01T00:00:00.000Z`.

Each response has one categorical band, expected NLCD codes, nodata 250, a matching 2246×2004 EPSG:5070 grid and nominal 30 m resolution within 1e-6 m serialization tolerance. A checker initially rejected exact float equality; inspection justified the explicit tolerance without resampling. Common valid cells: 4,500,984; changed cells 2005→2015: 636,199; 2015→2025: 531,866. Different arrays rule out identical-response failure but are not independent year authentication. MRLC advertises current Collection 1.2; the unversioned service/TIFF tags do not independently attest collection, so freeze hashes and confirm corresponding metadata before scientific release. Data cover the original-project AOI **bounding rectangle**, not its inspected/dissolved polygon. Bounding transformation was offline; geometry/edge QA remains. No dashboard baseline was replaced; no San Antonio fit or hindcast ran; **2025 remains held out**.

`scripts/report_nlcd_download.py --inputs FILE_2005 FILE_2015 FILE_2025 --output outputs/nlcd_new_qa` emits acquisition QA JSON (hashes, tags, class counts, request) and a separate report including failed attempts and limitations. Actual local reports live under `outputs/nlcd_download_retry_v1/`. Acquisition is no longer blocked; collection attestation, exact AOI and time-valid drivers remain gates before the proposed 2005→2015 / 2015→2025 validation.

**Software regression:** 49 PLUS-related unittest cases passed; the full dashboard gate completed with exit 0 and “All baselines match” (40 snapshots plus its acceptance assertions). It skipped archived ownership rule-output verification because that external GPKG is absent, and some baseline fields such as `schools_nature_access` dictionaries are not snapshot-compared; these limitations are inherited from the gate, not new scientific validation. No schema bump was required because dashboard math/outputs did not change. Separate local software and baseline reports are under `outputs/plus_tutorial_test_v1/`.
