...
### Ownership / parcel layer (Ownership Integration Commit 0.5)

Boolean masks on the SA grid keying ownership-constrained placement (`docs/internal/OWNERSHIP_INTEGRATION_SPEC.md`). The runtime artifact is the rasterized public/vacant mask; the 281 MB polygon audit trail is gitignored (Streamlit Cloud deploy weight) and archived outside the repo.

| City | Path | Status | Count / Notes |
|---|---|---|---|
| SA — public / vacant raster (legacy) | `data/sa/sa_public_vacant_30m.tif` | Committed | EPSG:5070; int8 raster on the SA grid (1713 × 1984 @ 30 m); codes -1 (outside) / 0 (neither) / 1 (public only) / 2 (vacant only) / 3 (public AND vacant). Built from a 710,772-record full-county BCAD pull on **2026-05-31** (711 pages, EOD-confirmed; one MISSING page recovered during the verify pass). **Vacancy methodology: exempt-keyed** — `State_cd C*` on any parcel, plus `ImprVal == 0` only on parcels NOT carrying an `EX-X*` total-exemption flag. The exempt-keyed rule replaces the naive `C* OR ImprVal == 0` union, which over-catches tax-exempt civic / church / university built parcels as "vacant" (improvements unassessed, not absent); see `docs/research/ownership/PHASE_0_INVESTIGATION.md` for the empirical per-token cluster behind the rule. **License: not explicitly stated; Bexar County GIS / BCAD; attribution cited.** Rebuild command: `python scripts/data/download_bexar_parcels.py --fetch && python scripts/data/download_bexar_parcels.py --finish` (Phase A ~22 min wall + Phase B classify+rasterize ~3 min). Status: legacy — still consumed by `OWNERSHIP_MODES` until Batch 2 of `docs/internal/OWNERSHIP_FINER_CLASSES_SPEC.md` lands. |
| SA — ownership 2-band raster (finer classes) | `data/sa/sa_ownership_2band_30m.tif` | Committed | EPSG:5070; **two-band int8** raster on the SA grid (1713 × 1984 @ 30 m). **Band 1 — ownership class enum (7 classes after the School / University Split):** `0=private`, `1=city`, `2=county`, `3=state-federal`, `4=school`, `5=unknown`, `6=university`; nodata `-1` (outside AOI). Class IDs are stable across batches — `private=0` and `unknown=5` stay fixed; `school` keeps code 4 (was `school_university`), `university` takes the new code 6. Classes are written last in priority order (low → high: unknown < private < university < state-federal < county < school < city) so government classes win at overlapping pixel edges; university (mixed public + private) sits below the four gov classes. **Rasterized sub-pixel → 30 m area-majority (2026-06-04).** Parcels are burned at 3× (10 m); each 30 m cell takes the area-majority owner class of the intersecting parcels (priority preserved at sub-pixel). This fixed a 16 %-of-population ownership-nodata 30 m grid-tiling artifact — interstitial cells between abutting parcels that the prior center-point single-pass burn left at `-1` (nodata pop 16.14 % → 0.97 %; 96.4 % of recovered pop → `private`; 0.17 % of previously-classified pixels shifted to their area-majority owner). A ~0.97 % residual (cells with no parcel present at 10 m — degenerate slivers, narrow non-parcel ROW, water/federal micro-gaps) stays nodata BY DESIGN; no inferential nearest-fill. See `DESIGN_NOTES.md` §6.8. **Band 2 — vacant flag:** `0=not-vacant`, `1=vacant`; nodata `-1`. Carried through the same sub-pixel priority burn, so a cell inherits the vacant flag of the parcel(s) it was assigned. Same exempt-keyed vacancy rule as the legacy raster. Source: 2026-05-31 BCAD pull, re-classified under the seven-way rule from `docs/internal/OWNERSHIP_FEASIBILITY_PROFILING.md` + the School / University Split addendum (HOA → Private filter on the County branch; standalone `\bUSA\b` and `\bFEDERAL\b` dropped from State-federal after the spot-check surfaced ~1.3k ac of business-name false positives — Boralis USA Inc, Forestar (USA), Home Depot USA Inc, Security Service Federal Credit Union; `\bSCHOOL\b` alone dropped from the school regex — would sweep in private schools/academies). Rebuild command: `python scripts/data/download_bexar_parcels.py --reclassify-from <archived.gpkg>` (re-rasterizes from the archived 2026-05-31 GPKG without re-fetching BCAD). **Per-class polygon-Acres (full Bexar County, locked by `verify_baselines.py` reconciliation at ±0.5%):** private 606,433 ac · city 126,634 · state-federal 54,883 · university 3,771 · county 3,018 · school 2,604 · unknown 1,735 · total 799,079 ac. **Per-class rasterized-AOI acres (locked at ±5%; re-snapshotted 2026-06-04 after the sub-pixel re-rasterization — every class grew as boundary slivers were recovered):** private ~553,301 · city ~42,409 · state-federal ~28,617 · unknown ~15,972 · university ~3,758 · county ~3,106 · school ~2,611. **School / University Split sanity:** new school + university polygon-Acres = 2,604 + 3,771 = 6,375 ac vs the pre-split combined 6,430 ac → 55 ac (0.86%, 20 parcels) fell through to `private` — names matching neither ISD/SCHOOL DISTRICT nor UNIVERSITY/COLLEGE (private K-12 schools, academies, charter operators not named ISD). Small population, as expected. **Caveat — BCAD `Acres` field ≠ polygon geometry for two classes.** Reported `Acres` and geometry-derived acres reconcile within ~2% for private / county / school / state-federal / university, but diverge meaningfully for City (the master "CITY OF SAN ANTONIO" record reports ~115k aggregated acres against a much smaller polygon footprint — BCAD overcounts) and Unknown (parcels with Owner blank are mostly road rights-of-way with `Acres=0` but real geometry — ~16k ac of in-AOI ROW). The raster reflects geometry; the polygon-Acres reconciliation reflects the BCAD field. **Caveat — rule-derived, not ground-truthed.** Classes come from regex-parsing BCAD `Owner` + `Exempts`. No cross-check against an authoritative title registry. The filter is a planning screen. **Caveat — `school` = K-12 public districts only.** The school regex is `\b(ISD\|INDEPENDENT SCHOOL DISTRICT\|SCHOOL DISTRICT)\b` — deliberately NOT keying on bare `\bSCHOOL\b` (which would sweep in private schools and academies). Charter operators not named ISD, private K-12 schools, and academies fall through to `private` (~55 ac / 20 parcels post-split — small population per the sanity check above). `school` IS in the `OWNERSHIP_MODES['public']` rollup — school districts are clearly government. **Caveat — `university` is mixed public + private.** The bucket includes public institutions (UT System, TX A&M System, Alamo Community College District) AND private institutions (Trinity University, St. Mary's University, Our Lady of the Lake University) — both caught by `\b(UNIVERSITY\|COLLEGE)\b`. Treat `university` as "college or university campuses" — NOT as a government category. For the "Publicly-owned land" rollup (`OWNERSHIP_MODES['public']`), `university` is intentionally excluded — only `city + county + state_federal + school` qualify, so a private Trinity campus doesn't get counted as publicly-available planning land. `university` remains a first-class selectable filter on its own. |
| SA — polygon audit trail | `data/sa/sa_ownership.gpkg` (gitignored) | Local-only | EPSG:5070; 710,772 polygons with columns `Owner / State_cd / ImprVal / Exempts / Acres / owner_class / is_public / is_totally_exempt / is_vacant`. 281 MB. Generated by the same script as the raster; written to `data/sa/` but excluded via `.gitignore`. Archived for future drill-down at `~/Desktop/ecosystem_explorer_archive/sa_ownership_bexar_2026-05-31.gpkg`; re-pull from BCAD if a fresh snapshot is needed. Note: the archived GPKG predates the six-way classifier; running the script's `--reclassify-from` against it applies the new rules to the same polygons and updates `data/sa/sa_ownership_2band_30m.tif`. |
