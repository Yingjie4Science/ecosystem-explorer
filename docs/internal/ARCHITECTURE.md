...
The approved PLUS hook is an out-of-process future-background branch: PLUS-native output is preserved, translated once into the canonical project LULC taxonomy, validated, and then exposed as a new scenario source. It is not part of the current runtime and will not execute inside a Streamlit rerun. The complete proposed boundary and release gates live in `PLUS_PRODUCTION_WORKFLOW.md`.
