...
The overwrite covers metrics that depend on per-rerun state (fresh rasters, cost sliders, carbon-rate sliders); fields loaded from the lookup (`nature_access_pct`, `children_nature_access_pct`, `children_with_nature_access`, `mean_hm`, `flood_reduction`, `runoff_acre_feet`, the MH metrics, the `n_wet`/`n_for`/`n_hd` counts, etc.) are schema-current because `SCENARIO_SCHEMA_VERSION` is part of the cache key — no defensive overwrite needed for those. When any non-random placement strategy is active, the lookup table is bypassed entirely and all metrics run live. The decision rationale lives in DESIGN_NOTES §4.
...
**Outputs.** The surrogate predicts six metrics: `flood_reduction`, `mean_hm`, `food_mln_lbs`, `runoff_acre_feet`, `carbon_tons_co2`, `nature_access_pct`. UMH metrics (`preventable_mh_cases`, `avoided_mh_cost_usd`) are computed deterministically from NDVI exposure and are not RF targets. Children's nature access (`children_nature_access_pct`, `children_with_nature_access`) is likewise computed deterministically — a child-reweighted UNA access share — not an RF target. See REFERENCE §6.
...
**Authoritative taxonomy lives in NATCAP_ALIGNMENT.md §2.** The four-state per-card badge vocabulary (`NatCap published value` / `≈ NatCap method` / `≈ Aligned method` / `Prototype`) plus the per-metric-evidence nuance (temperature and carbon both cite measured per-pixel parity; aligned-method metrics cannot) and the per-metric × per-context switch (NatCap published value fires only in the fixed-scenario reference view) are documented there. ARCHITECTURE §6 covers the *components* — what code renders the badges and where they're wired — and does NOT restate the taxonomy. See NATCAP_ALIGNMENT §2 for the badge meanings, REFERENCE §4 for the user-facing explanation, and DESIGN_NOTES §8 for the design rationale.
...
The PLUS branch stays offline and out of Streamlit reruns. `plus_workflow.py` / `scripts/run_plus_workflow.py` now implement the San Antonio engineering handoff: preparation, external PLUS-raster import or labeled fixture, grid/class/constraint checks, explicit compound expansion, matched interventions, existing-engine evaluation, canonical bundles and hashed review artifacts. Actual PLUS execution and calibrated projection release are not implemented; no new dashboard scenario source is exposed. The complete boundary, runnable commands and release gates live in `PLUS_PRODUCTION_WORKFLOW.md` §16.
...
An independent `plus_inspired/` package now provides a separate offline experimental engine: `core.py` owns array contracts, feasibility/patch allocation and validation; `model.py` owns expansion forests and safe JSON persistence; `runner.py` owns strict raster/config IO and receipts; `__main__.py` owns its CLI. It does not import the dashboard, change `evaluate_scenario`, or present itself as a native PLUS producer. `scripts/run_plus_inspired_demo.py` exercises it on manufactured data. The specification, deviations and release limits live in `PLUS_PRODUCTION_WORKFLOW.md` §16.10. Import into the existing official-PLUS adapter is not wired: its engine-record identity checks must not be bypassed.
